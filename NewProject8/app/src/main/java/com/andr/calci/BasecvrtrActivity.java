...
public class BasecvrtrActivity extends AppCompatActivity {
	
	private double size = 0;
	private double typesound = 0;
	private HashMap<String, Object> btns = new HashMap<>();
	private HashMap<String, Object> log = new HashMap<>();
	public Cryptography crypto = new Cryptography();
	
	private ArrayList<String> resDecompose = new ArrayList<>();
	...
	private TextView btnA1;
	private TextView btnB1;
	private TextView btnC1;
	private TextView btnD1;
	private TextView btnA2;
	private TextView btnB2;
	private TextView btnC2;
	private TextView btnD2;
	private TextView btnA3;
	private TextView btnB3;
	private TextView btnC3;
	private TextView btnD3;
	private TextView btnA4;
	private TextView btnB4;
	private TextView btnC4;
	private TextView btnD4;
	private LinearLayout linear23;
	private LinearLayout downloadlayout;
	private LinearLayout downloadlayout_fake;
	...
	protected void onCreate(Bundle _savedInstanceState) {
		...
		setContentView(R.layout.basecvrtr);
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		octlayout = findViewById(R.id.octlayout);
		declayout = findViewById(R.id.declayout);
		hexlayout = findViewById(R.id.hexlayout);
		binimg = findViewById(R.id.binimg);
		binTxt = findViewById(R.id.binTxt);
		octimg = findViewById(R.id.octimg);
		octTxt = findViewById(R.id.octTxt);
		decimg = findViewById(R.id.decimg);
		decTxt = findViewById(R.id.decTxt);
		heximg = findViewById(R.id.heximg);
		hexTxt = findViewById(R.id.hexTxt);
		...
		downloadlayout_fake = findViewById(R.id.downloadlayout_fake);
		downbar = findViewById(R.id.downbar);
		...
		binimg.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				if (log.get("base_current_dialog").toString().equals("none")) {
					log.put("input_type", "01");
					_setInputType();
				}
			}
		});
		
		octimg.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				if (log.get("base_current_dialog").toString().equals("none")) {
					log.put("input_type", "01234567");
					_setInputType();
				}
			}
		});
		...
		decimg.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				if (log.get("base_current_dialog").toString().equals("none")) {
					log.put("input_type", "0123456789");
					_setInputType();
				}
			}
		});
		
		heximg.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				if (log.get("base_current_dialog").toString().equals("none")) {
					log.put("input_type", "0123456789ABCDEF");
					_setInputType();
				}
			}
		});
		...
		btnclr.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				if (log.get("base_current_dialog").toString().equals("none")) {
					_sensor();
					if (log.get("input_type").toString().equals("01")) {
						binTxt.setText(binTxt.getText().toString().substring((int)(0), (int)((binTxt.getText().toString().length())-1)));
						if (binTxt.getText().toString().equals("")) {
							binTxt.setText("0");
						}
					}
					else {
						if (log.get("input_type").toString().equals("01234567")) {
							octTxt.setText(octTxt.getText().toString().substring((int)(0), (int)((octTxt.getText().toString().length())-1)));
							if (octTxt.getText().toString().equals("")) {
								octTxt.setText("0");
							}
						}
						else {
							if (log.get("input_type").toString().equals("0123456789")) {
								decTxt.setText(decTxt.getText().toString().substring((int)(0), (int)((decTxt.getText().toString().length())-1)));
								if (decTxt.getText().toString().equals("")) {
									decTxt.setText("0");
								}
							}
							else {
								if (log.get("input_type").toString().equals("0123456789ABCDEF")) {
									hexTxt.setText(hexTxt.getText().toString().substring((int)(0), (int)((hexTxt.getText().toString().length())-1)));
									if (hexTxt.getText().toString().equals("")) {
										hexTxt.setText("0");
									}
								}
							}
	...
	private void initializeLogic() {
		...
		binTxt.setInputType(0);
		octTxt.setInputType(0);
		decTxt.setInputType(0);
		hexTxt.setInputType(0);
		/*
binTxt.setTextIsSelectable(true);
octTxt.setTextIsSelectable(true);
decTxt.setTextIsSelectable(true);
hexTxt.setTextIsSelectable(true);
hexTxt.setFilters(new InputFilter[]{new InputFilter.LengthFilter((int) 7)});
*/
		binTxt.setFilters(new InputFilter[]{new InputFilter.LengthFilter((int) 28)});
		binTxt.setOnTouchListener(new View.OnTouchListener(){
			@Override
		...
		octTxt.setOnTouchListener(new View.OnTouchListener(){
			@Override
			public boolean onTouch(View _touchedView , MotionEvent event){
				int ev = event.getAction();
				switch (ev) {
		...
		decTxt.setOnTouchListener(new View.OnTouchListener(){
			@Override
			public boolean onTouch(View _touchedView , MotionEvent event){
				int ev = event.getAction();
				switch (ev) {
		...
		hexTxt.setOnTouchListener(new View.OnTouchListener(){
			@Override
			public boolean onTouch(View _touchedView , MotionEvent event){
				int ev = event.getAction();
				switch (ev) {
	...
	public void onResume() {
		...
		if (!FileUtil.isExistFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/tools/base_converter/source.dat"))) {
			FileUtil.writeFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/tools/base_converter/source.dat"), "");
		}
		if (!FileUtil.isExistFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/tools/base_converter/resource.dat"))) {
			FileUtil.writeFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/tools/base_converter/resource.dat"), "");
		}
		if (FileUtil.readFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/tools/base_converter/source.dat")).equals(crypto.encrypt(local.getString("base_converter", ""),local.getString("digikey", "")))) {
			_drawbtn();
		}
		else {
			_download();
	...
	public void onBackPressed() {
		...
		Animatoo.animateSwipeRight(BasecvrtrActivity.this);
		finish();
	}
	...
	public void _design() {
		...
		size = (SketchwareUtil.getDisplayHeightPixels(getApplicationContext()) - 200) / 7;
		...
		//layout Sizing
		btnA1.getLayoutParams().width=(int)((int) (size));
		btnA1.getLayoutParams().height=(int)((int) (size));
		btnA2.getLayoutParams().width=(int)((int) (size));
		btnA2.getLayoutParams().height=(int)((int) (size));
		btnA3.getLayoutParams().width=(int)((int) (size));
		btnA3.getLayoutParams().height=(int)((int) (size));
		btnA4.getLayoutParams().width=(int)((int) (size));
		btnA4.getLayoutParams().height=(int)((int) (size));
		btnB1.getLayoutParams().width=(int)((int) (size));
		btnB1.getLayoutParams().height=(int)((int) (size));
		btnB2.getLayoutParams().width=(int)((int) (size));
		btnB2.getLayoutParams().height=(int)((int) (size));
		btnB3.getLayoutParams().width=(int)((int) (size));
		btnB3.getLayoutParams().height=(int)((int) (size));
		btnB4.getLayoutParams().width=(int)((int) (size));
		btnB4.getLayoutParams().height=(int)((int) (size));
		btnC1.getLayoutParams().width=(int)((int) (size));
		btnC1.getLayoutParams().height=(int)((int) (size));
		btnC2.getLayoutParams().width=(int)((int) (size));
		btnC2.getLayoutParams().height=(int)((int) (size));
		btnC3.getLayoutParams().width=(int)((int) (size));
		btnC3.getLayoutParams().height=(int)((int) (size));
		btnC4.getLayoutParams().width=(int)((int) (size));
		btnC4.getLayoutParams().height=(int)((int) (size));
		btnD1.getLayoutParams().width=(int)((int) (size));
		btnD1.getLayoutParams().height=(int)((int) (size));
		btnD2.getLayoutParams().width=(int)((int) (size));
		btnD2.getLayoutParams().height=(int)((int) (size));
		btnD3.getLayoutParams().width=(int)((int) (size));
		btnD3.getLayoutParams().height=(int)((int) (size));
		btnD4.getLayoutParams().width=(int)((int) (size));
		btnD4.getLayoutParams().height=(int)((int) (size));
		btnclr.getLayoutParams().width=(int)((int) (size));
		btnclr.getLayoutParams().height=(int)((int) (size));
		binimg.requestLayout();
		binimg.getLayoutParams().height = (int) (size / 2);
		binimg.getLayoutParams().width = (int) (size / 2);
		octimg.requestLayout();
		octimg.getLayoutParams().height = (int) (size / 2);
		octimg.getLayoutParams().width = (int) (size / 2);
		decimg.requestLayout();
		decimg.getLayoutParams().height = (int) (size / 2);
		decimg.getLayoutParams().width = (int) (size / 2);
		heximg.requestLayout();
		heximg.getLayoutParams().height = (int) (size / 2);
		heximg.getLayoutParams().width = (int) (size / 2);
		LinearLayout linear3 = (LinearLayout) findViewById(R.id.linear3);
		linear3.getLayoutParams().width=(int)((int) (size * 4));
		linear3.getLayoutParams().height=(int)(android.widget.LinearLayout.LayoutParams.WRAP_CONTENT);
		LinearLayout linear22 = (LinearLayout) findViewById(R.id.linear22);
		linear22.getLayoutParams().width=(int)((int) ((size * 4) + 100));
		linear22.getLayoutParams().height=(int)(android.widget.LinearLayout.LayoutParams.WRAP_CONTENT);
		//rounding corners
		binlayout.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b, int c, int d) { this.setCornerRadius(a); this.setStroke(b, c); this.setColor(d); return this; } }.getIns((int)999, (int)2, 0x88888888, 0xFF000000));
		octlayout.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b, int c, int d) { this.setCornerRadius(a); this.setStroke(b, c); this.setColor(d); return this; } }.getIns((int)999, (int)2, 0x88888888, 0xFF000000));
		declayout.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b, int c, int d) { this.setCornerRadius(a); this.setStroke(b, c); this.setColor(d); return this; } }.getIns((int)999, (int)2, 0x88888888, 0xFF000000));
		hexlayout.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b, int c, int d) { this.setCornerRadius(a); this.setStroke(b, c); this.setColor(d); return this; } }.getIns((int)999, (int)2, 0x88888888, 0xFF000000));
		btnA1.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0x88888888));
		btnA2.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0x88888888));
		btnA3.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0x88888888));
		btnA4.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0x88888888));
		btnB1.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0x88888888));
		btnB2.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0x88888888));
		btnB3.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0x88888888));
		btnB4.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0x88888888));
		btnC1.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0x88888888));
		btnC2.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0x88888888));
		btnC3.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0x88888888));
		btnC4.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0x88888888));
		btnD1.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0x88888888));
		btnD2.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0x88888888));
		btnD3.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0x88888888));
		btnD4.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0x88888888));
		btnclr.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0xFFFF9800));
		downtext.setTextSize((int)TypedValue.COMPLEX_UNIT_PX,(int)((size*4)/24));
		btnA1.setTextSize((int)TypedValue.COMPLEX_UNIT_PX,(int)(size/2));
		btnA2.setTextSize((int)TypedValue.COMPLEX_UNIT_PX,(int)(size/2));
		btnA3.setTextSize((int)TypedValue.COMPLEX_UNIT_PX,(int)(size/2));
		btnA4.setTextSize((int)TypedValue.COMPLEX_UNIT_PX,(int)(size/2));
		btnB1.setTextSize((int)TypedValue.COMPLEX_UNIT_PX,(int)(size/2));
		btnB2.setTextSize((int)TypedValue.COMPLEX_UNIT_PX,(int)(size/2));
		btnB3.setTextSize((int)TypedValue.COMPLEX_UNIT_PX,(int)(size/2));
		btnB4.setTextSize((int)TypedValue.COMPLEX_UNIT_PX,(int)(size/2));
		btnC1.setTextSize((int)TypedValue.COMPLEX_UNIT_PX,(int)(size/2));
		btnC2.setTextSize((int)TypedValue.COMPLEX_UNIT_PX,(int)(size/2));
		btnC3.setTextSize((int)TypedValue.COMPLEX_UNIT_PX,(int)(size/2));
		btnC4.setTextSize((int)TypedValue.COMPLEX_UNIT_PX,(int)(size/2));
		btnD1.setTextSize((int)TypedValue.COMPLEX_UNIT_PX,(int)(size/2));
		btnD2.setTextSize((int)TypedValue.COMPLEX_UNIT_PX,(int)(size/2));
		btnD3.setTextSize((int)TypedValue.COMPLEX_UNIT_PX,(int)(size/2));
		btnD4.setTextSize((int)TypedValue.COMPLEX_UNIT_PX,(int)(size/2));
		btnclr.setTextSize((int)TypedValue.COMPLEX_UNIT_PX,(int)(size/3));
		binTxt.setTextSize((int)TypedValue.COMPLEX_UNIT_PX,(int)(size/4));
		octTxt.setTextSize((int)TypedValue.COMPLEX_UNIT_PX,(int)(size/4));
		decTxt.setTextSize((int)TypedValue.COMPLEX_UNIT_PX,(int)(size/4));
		hexTxt.setTextSize((int)TypedValue.COMPLEX_UNIT_PX,(int)(size/4));
		binimg.setImageResource(R.drawable.bin_disabled);
		octimg.setImageResource(R.drawable.oct_disabled);
		decimg.setImageResource(R.drawable.dec_disabled);
		heximg.setImageResource(R.drawable.hex_disabled);
	}
	
	
	public void _logy(final String _str) {
		downloadlayout.setVisibility(View.GONE);
		downloadlayout_fake.setVisibility(View.GONE);
		if (_str.equals("dismiss")) {
			screen.setZ((float) (1));
			disability.setZ((float) (-1));
			log.put("base_current_dialog", "none");
		}
		else {
			disability.setZ((float) (1));
			screen.setZ((float) (-1));
			if (_str.equals("download")) {
				log.put("base_current_dialog", "download");
	...
	public void _download() {
		FileUtil.deleteFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/tools/base_converter/source.dat"));
		FileUtil.deleteFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/tools/base_converter/resource.dat"));
		downtext.setText("");
		_logy("download");
		PRDownloaderConfig config = PRDownloaderConfig.newBuilder()
			.setDatabaseEnabled(true)
			.build();
		PRDownloader.initialize(BasecvrtrActivity.this, config);
		button1.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
					...
					downloadId = PRDownloader.download(local.getString("base_converter", "")
				, FileUtil.getPackageDataDir(getApplicationContext()).concat("/tools/base_converter/"), "resource.dat")
						.build()
					...
					.setOnProgressListener(new OnProgressListener() {
						...
						downbar.setIndeterminate(false);
						progressbar2.setProgress((int)progressPercent);
						downbar.setProgress((int)progressPercent);
						downtext.setText(String.valueOf((long)(progressPercent)).concat("%"));
					...
					.start(new OnDownloadListener() {
						...
						FileUtil.writeFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/tools/base_converter/resource.dat"), crypto.encrypt(FileUtil.readFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/tools/base_converter/resource.dat")),local.getString("digikey", "")));
						FileUtil.writeFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/tools/base_converter/source.dat"), crypto.encrypt(local.getString("base_converter", ""),local.getString("digikey", "")));
						_logy("dismiss");
						_drawbtn();
							}
							@Override
	...
	public void _drawbtn() {
		if (crypto.decrypt(FileUtil.readFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/tools/base_converter/resource.dat")),local.getString("digikey", "")).equals("")) {
			_download();
		}
		else {
			btns.clear();
			btns = new Gson().fromJson(crypto.decrypt(FileUtil.readFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/tools/base_converter/resource.dat")),local.getString("digikey", "")), new TypeToken<HashMap<String, Object>>(){}.getType());
			btnA1.setText(btns.get("A1").toString());
			btnA2.setText(btns.get("A2").toString());
			btnA3.setText(btns.get("A3").toString());
			btnA4.setText(btns.get("A4").toString());
			btnB1.setText(btns.get("B1").toString());
			btnB2.setText(btns.get("B2").toString());
			btnB3.setText(btns.get("B3").toString());
			btnB4.setText(btns.get("B4").toString());
			btnC1.setText(btns.get("C1").toString());
			btnC2.setText(btns.get("C2").toString());
			btnC3.setText(btns.get("C3").toString());
			btnC4.setText(btns.get("C4").toString());
			btnD1.setText(btns.get("D1").toString());
			btnD2.setText(btns.get("D2").toString());
			btnD3.setText(btns.get("D3").toString());
			btnD4.setText(btns.get("D4").toString());
			if (!log.containsKey("input_type")) {
				decimg.performClick();
			}
		}
	}
	...
	public void _setInputType() {
		_design();
		_sensor();
		if (log.get("input_type").toString().equals("01")) {
			binlayout.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b, int c, int d) { this.setCornerRadius(a); this.setStroke(b, c); this.setColor(d); return this; } }.getIns((int)999, (int)2, 0xFFFF9800, 0xFF000000));
			binimg.setImageResource(R.drawable.bin_enabled);
		}
		else {
			if (log.get("input_type").toString().equals("01234567")) {
				octlayout.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b, int c, int d) { this.setCornerRadius(a); this.setStroke(b, c); this.setColor(d); return this; } }.getIns((int)999, (int)2, 0xFFFF9800, 0xFF000000));
				octimg.setImageResource(R.drawable.oct_enabled);
			}
			else {
				if (log.get("input_type").toString().equals("0123456789")) {
					declayout.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b, int c, int d) { this.setCornerRadius(a); this.setStroke(b, c); this.setColor(d); return this; } }.getIns((int)999, (int)2, 0xFFFF9800, 0xFF000000));
					decimg.setImageResource(R.drawable.dec_enabled);
				}
				else {
					if (log.get("input_type").toString().equals("0123456789ABCDEF")) {
						hexlayout.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b, int c, int d) { this.setCornerRadius(a); this.setStroke(b, c); this.setColor(d); return this; } }.getIns((int)999, (int)2, 0xFFFF9800, 0xFF000000));
						heximg.setImageResource(R.drawable.hex_enabled);
					}
				}
			}
		}
		if (log.get("input_type").toString().contains(btnA1.getText().toString())) {
			btnA1.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0xFFFF9800));
		}
		if (log.get("input_type").toString().contains(btnA2.getText().toString())) {
			btnA2.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0xFFFF9800));
		}
		if (log.get("input_type").toString().contains(btnA3.getText().toString())) {
			btnA3.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0xFFFF9800));
		}
		if (log.get("input_type").toString().contains(btnA4.getText().toString())) {
			btnA4.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0xFFFF9800));
		}
		if (log.get("input_type").toString().contains(btnB1.getText().toString())) {
			btnB1.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0xFFFF9800));
		}
		if (log.get("input_type").toString().contains(btnB2.getText().toString())) {
			btnB2.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0xFFFF9800));
		}
		if (log.get("input_type").toString().contains(btnB3.getText().toString())) {
			btnB3.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0xFFFF9800));
		}
		if (log.get("input_type").toString().contains(btnB4.getText().toString())) {
			btnB4.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0xFFFF9800));
		}
		if (log.get("input_type").toString().contains(btnC1.getText().toString())) {
			btnC1.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0xFFFF9800));
		}
		if (log.get("input_type").toString().contains(btnC2.getText().toString())) {
			btnC2.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0xFFFF9800));
		}
		if (log.get("input_type").toString().contains(btnC3.getText().toString())) {
			btnC3.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0xFFFF9800));
		}
		if (log.get("input_type").toString().contains(btnC4.getText().toString())) {
			btnC4.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0xFFFF9800));
		}
		if (log.get("input_type").toString().contains(btnD1.getText().toString())) {
			btnD1.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0xFFFF9800));
		}
		if (log.get("input_type").toString().contains(btnD2.getText().toString())) {
			btnD2.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0xFFFF9800));
		}
		if (log.get("input_type").toString().contains(btnD3.getText().toString())) {
			btnD3.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0xFFFF9800));
		}
		if (log.get("input_type").toString().contains(btnD4.getText().toString())) {
			btnD4.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)999, 0xFFFF9800));
		}
	}
	...
	public void _convert() {
		...
		try{
			if (log.get("input_type").toString().equals("01")) {
				num = Integer.parseInt(binTxt.getText().toString(),2);
				decTxt.setText(Integer.toString(num));
				num = Integer.parseInt(decTxt.getText().toString());;
				octTxt.setText(Integer.toOctalString(num));
				hexTxt.setText(Integer.toHexString(num));
			}
			else {
				if (log.get("input_type").toString().equals("01234567")) {
					num = Integer.parseInt(octTxt.getText().toString(),8);
					decTxt.setText(Integer.toString(num));
					num = Integer.parseInt(decTxt.getText().toString());;
					binTxt.setText(Integer.toBinaryString(num));
					hexTxt.setText(Integer.toHexString(num));
				}
				else {
					if (log.get("input_type").toString().equals("0123456789")) {
						...
						if (log.get("input_type").toString().equals("0123456789ABCDEF")) {
							num = Integer.parseInt(hexTxt.getText().toString(),16);
							decTxt.setText(Integer.toString(num));
							num = Integer.parseInt(decTxt.getText().toString());;
							binTxt.setText(Integer.toBinaryString(num));
							octTxt.setText(Integer.toOctalString(num));
			...
			btnclr.performClick();
			SketchwareUtil.showMessage(getApplicationContext(), "The expression is too long to convert");
		}
		hexTxt.setText(hexTxt.getText().toString().replace("a", "A"));
		hexTxt.setText(hexTxt.getText().toString().replace("b", "B"));
		hexTxt.setText(hexTxt.getText().toString().replace("c", "C"));
		hexTxt.setText(hexTxt.getText().toString().replace("d", "D"));
		hexTxt.setText(hexTxt.getText().toString().replace("e", "E"));
		hexTxt.setText(hexTxt.getText().toString().replace("f", "F"));
	}
	
	
	public void _clicked(final TextView _txtview) {
		if (log.get("base_current_dialog").toString().equals("none")) {
			if (log.get("input_type").toString().contains(_txtview.getText().toString())) {
				_sensor();
				if (log.get("input_type").toString().equals("01")) {
					if (binTxt.getText().toString().equals("0")) {
						binTxt.setText(_txtview.getText().toString());
					}
					else {
						binTxt.setText(binTxt.getText().toString().concat(_txtview.getText().toString()));
					}
				}
				else {
					if (log.get("input_type").toString().equals("01234567")) {
						if (octTxt.getText().toString().equals("0")) {
							octTxt.setText(_txtview.getText().toString());
						}
						else {
							octTxt.setText(octTxt.getText().toString().concat(_txtview.getText().toString()));
						}
					}
					else {
						if (log.get("input_type").toString().equals("0123456789")) {
							if (decTxt.getText().toString().equals("0")) {
								decTxt.setText(_txtview.getText().toString());
							}
							else {
								decTxt.setText(decTxt.getText().toString().concat(_txtview.getText().toString()));
							}
						}
						else {
							if (log.get("input_type").toString().equals("0123456789ABCDEF")) {
								if (hexTxt.getText().toString().equals("0")) {
									hexTxt.setText(_txtview.getText().toString());
								}
								else {
									hexTxt.setText(hexTxt.getText().toString().concat(_txtview.getText().toString()));
								}
							}
						}
					}
